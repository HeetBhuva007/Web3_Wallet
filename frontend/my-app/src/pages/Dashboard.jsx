...
// // --- MOCK API & REDUX FUNCTIONS ---
// //const mockGetUser = () => new Promise(resolve => setTimeout(() => resolve({ name: 'dev_user' }), 800));
// const mockGetPrivateKey = (publicKey) => new Promise(resolve => setTimeout(() => resolve(`0x_decrypted_privkey_${publicKey.slice(2, 10)}_${Date.now()}`), 1000));
...
//   const {user}=useSelector(state=>state.auth)
//   const dispatch=useDispatch()
//   // --- STATE FOR ALL MODALS AND TOASTS ---
//   const [airdropModalState, setAirdropModalState] = useState({ isOpen: false, account: null });
//   const [isAirdropping, setIsAirdropping] = useState(false);
//   const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
//   const [isCreatingAccount, setIsCreatingAccount] = useState(false);
...
//       const fetchedAccounts = accountsResponse.data.accounts || [];
//       setAccounts(fetchedAccounts);
//     //   fetchedAccounts.forEach(account => fetchBalanceForAccount(account.publicKey));
//     await Promise.all(
//         fetchedAccounts.map(account => fetchBalanceForAccount(account.publicKey))
//       );
//     } catch (err) {
...
//       setBalances(prev => ({ ...prev, [publicKey]: balance }));
//     } catch (err) { setBalances(prev => ({ ...prev, [publicKey]: 'Error' }));
//     } finally { setLoadingStates(prev => ({ ...prev, [`balance-${publicKey}`]: false })); }
//   };

//   const handleViewPrivateKey = async (publicKey) => {
//     if (revealedKeys[publicKey]) { setRevealedKeys(prev => ({ ...prev, [publicKey]: null })); return; }
//     setLoadingStates(prev => ({ ...prev, [`key-${publicKey}`]: true }));
//     try { 
//         const privateKey = await axiosClient.post(); 
//         setRevealedKeys(prev => ({ ...prev, [publicKey]: privateKey }));
//     } 
//     finally { setLoadingStates(prev => ({ ...prev, [`key-${publicKey}`]: false })); }
//   };
...
//     setIsCreatingAccount(true);
//     try {
//         const walletId = accounts[0]?.walletId||tempWalletId?._id; // Get walletId from existing account
//         if (!walletId) {
//             setToast({ message: "Cannot create account: Wallet ID not found.", type: 'error' });
//             setIsCreatingAccount(false);
//             return;
//         }
//       await axiosClient.post('/wallet/createAccount', {
//         walletId,
...
//     const { fromAccount } = sendModalState;
//     const { toPublicKey, amount, password } = sendForm;
//     if (!fromAccount || !toPublicKey || !amount || !password) { setToast({ message: 'All fields are required.', type: 'error' }); return; }
//     setIsSending(true);
//     try {
//         console.log(fromAccount)
//       await axiosClient.post('/transaction/sendSol', { 
...
//         {accounts.map((account, index) => (
//           <AccountCard
//             key={account.publicKey} account={account} index={index} balance={balances[account.publicKey]}
//             revealedKey={revealedKeys[account.publicKey]} loadingStates={loadingStates}
//             onViewKey={handleViewPrivateKey} onAirdrop={() => handleOpenAirdropModal(account)}
//             onSend={() => handleOpenSendModal(account)} onCopy={handleCopy}
//           />
//         ))}
//       </div>
//     );
...
//       <div className="max-w-7xl mx-auto">
//         <header className="flex flex-col items-start gap-4 mb-8 sm:flex-row sm:items-center sm:justify-between">
//             <div><div className="flex items-center gap-1 text-3xl font-bold text-white"><User size={30} className="text-brand-red"/><h2>{user ? `${user.userName}'s Dashboard` : 'Dashboard'}</h2></div><p className="mt-1 text-sm text-gray-400">Manage your accounts and assets.</p></div>
//             <div className='flex gap-3'>
//             <button onClick={handleOpenCreateModal} className="flex items-center  px-4 py-2 text-sm rounded-lg shrink-0 backpack-button"><Plus size={16} />Create Account</button>
//             <button onClick={handleLogout} className="flex items-center  px-4 py-2 text-sm rounded-lg shrink-0 backpack-button">Logout</button>
//             </div>
//         </header>
//         <main>{renderContent()}</main>
//       </div>
...
// const AccountCard = ({ account, index, balance, revealedKey, loadingStates, onViewKey, onAirdrop, onSend, onCopy }) => {
//   const isBalanceLoading=loadingStates[`balance-${account.publicKey}`];
//   const isKeyLoading=loadingStates[`key-${account.publicKey}`];
//   return(
//     <div className="flex flex-col h-full transition-all duration-300 backpack-card">
//       <div className="flex items-center justify-between p-4"><h3 className="font-bold text-white">Account #{index + 1}</h3>{getNetworkIcon(account.publicKey)}</div>
//       <div className="h-px bg-border-color" />
//       <div className="flex flex-col items-center justify-center flex-grow p-6 text-center">
//         <div className="text-4xl font-bold text-white">{isBalanceLoading ? <LoaderCircle size={36} className="animate-spin text-brand-red" /> : balance || '...'}</div>
//         <div className="flex items-center gap-2 mt-4 text-xs text-gray-400"><span>{truncateAddress(account.publicKey)}</span><Copy size={14} className="cursor-pointer shrink-0 hover:text-white" onClick={()=>onCopy(account.publicKey)} /></div>
//       </div>
//       <div className="h-px bg-border-color" />
//       <div className="flex items-center justify-around p-3">
//         <ActionButton icon={<Send size={20} />} label="Send" onClick={onSend} />
//         <ActionButton icon={<Droplets size={20} />} label="Airdrop" onClick={onAirdrop} />
//         <ActionButton icon={isKeyLoading ? <LoaderCircle size={20} className="animate-spin" /> : <KeyRound size={20} />} label={revealedKey ? 'Hide' : 'Key'} onClick={()=>onViewKey(account.publicKey)} disabled={isKeyLoading} isDanger={true} />
//       </div>
//       {revealedKey && (
//         <div className="p-3 mx-4 mb-4 text-xs bg-black/50 rounded-lg border-l-2 border-brand-red">
//           <div className="flex items-center justify-between"><span className="font-bold text-brand-red">PRIVATE KEY</span><Copy size={14} className="cursor-pointer hover:text-white" onClick={()=>onCopy(revealedKey)} /></div>
//           <p className="mt-2 break-all text-gray-300">{revealedKey}</p>
//         </div>
//       )}
//     </div>
...
//                 <p className="mt-2 text-sm text-center text-gray-400">Confirm your wallet password to authorize this action.</p>
//                 <form onSubmit={handleSubmit} className="mt-6 space-y-4">
//                     <div className="relative"><Lock className="absolute left-3.5 top-1/2 -translate-y-1/2 h-5 w-5 text-gray-500"/><input id="password" type="password" placeholder="Enter your password" value={password} onChange={(e)=>setPassword(e.target.value)} className="w-full pl-12 pr-4 py-3 text-sm rounded-lg backpack-input" required/></div>
//                     <div className="flex justify-center gap-4"><button type="button" onClick={onClose} disabled={isLoading} className="w-full px-4 py-2 text-sm font-semibold bg-gray-600 rounded-lg hover:bg-gray-500 disabled:opacity-50">Cancel</button><button type="submit" disabled={isLoading||!password} className="flex items-center justify-center w-full px-4 py-2 text-sm font-semibold text-white rounded-lg backpack-button disabled:opacity-50">{isLoading?<LoaderCircle size={20} className="animate-spin"/>:'Confirm Create'}</button></div>
//                 </form>
//             </div>
//         </div>
//     );
...
//             <button onClick={onClose} className="absolute -top-1 -right-1"><X size={14} className="p-0.5 bg-gray-800 rounded-full" /></button>
//         </div>
//     );
// };
...
//         {[...Array(3)].map((_, i) => (
//             <div key={i} className="flex flex-col h-full backpack-card animate-pulse">
//                 <div className="flex items-center justify-between p-4"><div className="w-24 h-5 bg-gray-700 rounded-md"></div><div className="w-6 h-6 bg-gray-700 rounded-full"></div></div>
//                 <div className="h-px bg-border-color" />
//                 <div className="flex flex-col items-center justify-center flex-grow p-6"><div className="w-32 h-10 bg-gray-700 rounded-md"></div><div className="w-24 h-4 mt-4 bg-gray-700 rounded-md"></div></div>
//                 <div className="h-px bg-border-color" />
//                 <div className="flex items-center justify-around p-3"><div className="w-12 h-10 bg-gray-700 rounded-md"></div><div className="w-12 h-10 bg-gray-700 rounded-md"></div><div className="w-12 h-10 bg-gray-700 rounded-md"></div></div>
//             </div>
//         ))}
//     </div>
// );
